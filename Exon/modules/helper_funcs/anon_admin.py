        ...
        def awrapper(update: Update, context: CallbackContext, *args, **kwargs):
            ...
            if is_anon:
                ...
                anon_callback_messages[(message.chat.id, message.message_id)] = (
                    message.reply_text(
                        "sᴇᴇᴍs ʟɪᴋᴇ you're ᴀɴᴏɴʏᴍᴏᴜs, ᴄʟɪᴄᴋ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ᴛᴏ ᴘʀᴏᴠᴇ ʏᴏᴜʀ ɪᴅᴇɴᴛɪᴛʏ",
...
async def anon_callback_handler1(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def resolve_user(user, message_id, chat):
    if user.id == 1087968824:
        try:
            ...
            return application.bot.edit_message_text(
                chat.id,
                message_id,
                "ʏᴏᴜ'ʀᴇ ɴᴏᴡ ɪᴅᴇɴᴛɪғɪᴇᴅ ᴀs -: {}".format(user.first_name),
            )
        except BaseException as e:
            return application.bot.edit_message_text(chat.id, message_id, f"ᴇʀʀᴏʀ: {e}")
