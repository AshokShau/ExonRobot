...
async def cb_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        await msg.reply_text("ᴘʀᴏᴠɪᴅᴇ sᴏᴍᴇ ᴛᴇʀᴍ ᴛᴏ sᴇᴀʀᴄʜ ғᴏʀ ᴀ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ.")
        return
    if len(query) > 50:
        await msg.reply_text("ᴘʀᴏᴠɪᴅᴇ ᴀ sᴇᴀʀᴄʜ ǫᴜᴇʀʏ ᴜɴᴅᴇʀ 50 ᴄʜᴀʀᴀᴄᴛᴇʀs")
        return
    if msg.from_user:
...
async def kang(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.reply_to_message:
        ...
        try:
            ...
            await msg.reply_text(" sᴏʀʀʏ ɪ ᴄᴀɴ'ᴛ ᴋᴀɴɢ ᴛʜᴀᴛ.")
            print(e)
            return
        except TelegramError as e:
            if e.message == "Stickerset_invalid":
