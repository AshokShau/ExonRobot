...
async def extract_user(
    ...
    if reply_message and reply_message.from_user:
        ...
        return user.id, user.first_name, user.username, m.text.split(None, 1)[1]

    entities = list(m.parse_entities([MessageEntity.TEXT_MENTION]))
    if entities:
        ent = entities[0]
        if ent.offset == len(m.text) - len(m.text.split(None, 1)[1]):
            return ent.user.id, ent.user.first_name, ent.user.username, m.text[ent.offset + ent.length :]
    ...
    if len(args) >= 1 and args[0].isdigit():
        ...
        try:
            ...
            return user.id, user.first_name, user.username, m.text.split(None, 2)[2]
        except BadRequest:
            return None, None, None, None

    if sender:
        return m.from_user.id, m.from_user.first_name, m.from_user.username, m.text.split(None, 1)[1]
