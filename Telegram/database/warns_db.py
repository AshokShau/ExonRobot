...
class WarnsDB:
    ...
    async def get_warns(self, user_id: int) -> Dict[str, Union[List[str], int]]:
        ...
        try:
            ...
            LOGGER.error(f"Error retrieving warns for user {user_id} in chat {self.chat_id}: {e}")
            return default_data

    async def warn_user(self, user_id: int, warn_reason: str = None) -> Dict[str, Union[List[str], int]]:
        try:
            warn_data = await self.collection.find_one({"_id": self.chat_id}) or {
                "warned_users": {}, "warn_limit": 5, "warn_mode": "none"
            }
            ...
            LOGGER.info(f"User {user_id} warned in chat {self.chat_id}. Total warns: {len(user_warnings)}")
    ...
    async def remove_warn(self, user_id: int) -> Dict[str, Union[List[str], int]]:
        try:
            ...
            LOGGER.info(f"Warn removed for user {user_id} in chat {self.chat_id}. Total warns: {len(user_warnings)}")
            return {"warns": user_warnings, "num_warns": len(user_warnings)}
        except PyMongoError as e:
            LOGGER.error(f"Error removing warn for user {user_id} in chat {self.chat_id}: {e}")
            return {"warns": [], "num_warns": 0}
    ...
    async def reset_warns(self, user_id: int) -> bool:
        try:
            ...
            LOGGER.error(f"Error resetting warns for user {user_id} in chat {self.chat_id}: {e}")
            return False
    ...
    async def get_warn_settings(self) -> Dict[str, Union[str, int]]:
        ...
        try:
            ...
            } if warn_data else default_settings
        except PyMongoError as e:
    ...
    async def set_warn_settings(self, warn_mode: str, warn_limit: int) -> bool:
        try:
            ...
            LOGGER.info(f"Warn settings updated for chat {self.chat_id}: mode={warn_mode}, limit={warn_limit}")
            return True
        except PyMongoError as e:
